...
Next, we wanted to look at how basketball develops within the span of a single game. In doing so, we
wanted to look at the average shot distance by quarter. We can use this visualization to examine how the state of the game determines a player/team's shot choice.
```{r}
...
for (i in 1:5) {
  ...
  print(paste0("Quarter ", i, " Mean Shot Distance: ", mean(subset$shot_dist)))
}


par(mfrow = c(1,1))
```
Since the time remaining in the game might affect where players shoot from, we thought it would be appropriate to graph how players start to shoot based on which period of the game 
the game is in. The graph is a box and whiskers plot that analyses the disrtibution of where players shot from based on which of the 5 possible phases the game could be in.
